...
def get_default_save_dir() -> Path:
    """Get the default save directory, with platform-appropriate locations.
    
    On Windows, tries locations in order:
    1. %LOCALAPPDATA%/pychess/saves (Windows standard for app data)
    2. %APPDATA%/pychess/saves (Windows roaming fallback)
    3. ~/pychess/saves (home directory without dot-prefix)
    4. ./pychess_saves (current directory as last resort)
    
    On Unix/Linux/macOS:
    1. ~/.pychess/saves (Unix-style hidden directory)
    2. ./pychess_saves (current directory as last resort)
    
    Returns:
        Path to a writable save directory
    """
    candidates = []
    
    if sys.platform == "win32":
        # Windows: Prefer standard Windows app data locations
        # These avoid dot-prefixed directories which can be blocked by corporate policies
        
        # LOCALAPPDATA is preferred for app data on Windows (e.g., C:\Users\X\AppData\Local)
        local_app_data = os.environ.get("LOCALAPPDATA")
        if local_app_data:
            candidates.append(Path(local_app_data) / "pychess" / "saves")
        
        # APPDATA as another fallback (e.g., C:\Users\X\AppData\Roaming)
        app_data = os.environ.get("APPDATA")
        if app_data:
            candidates.append(Path(app_data) / "pychess" / "saves")
        
        # Home directory without dot-prefix (some corporate policies block dot-directories)
        candidates.append(Path.home() / "pychess" / "saves")
    else:
        # Unix/Linux/macOS: Use standard Unix hidden directory convention
        candidates.append(Path.home() / ".pychess" / "saves")
    
    # Last resort for all platforms: current working directory
    candidates.append(Path.cwd() / "pychess_saves")
    ...
    raise PermissionError(
        "Could not create save directory. Tried:\n" +
        "\n".join(f"  - {c}" for c in candidates) +
        "\n\nPlease create one of these directories manually or run as administrator."
    )
...
class SaveManager:
    ...
    def __init__(self, save_dir: Optional[Path] = None) -> None:
        ...
        Args:
            save_dir: Directory for save files. Defaults to ~/.pychess/saves/
                      with automatic fallback to other locations on Windows
    ...
    def _ensure_save_dir(self) -> None:
        ...
        try:
            ...
            # On Windows, try fallback locations
            if sys.platform == "win32":
